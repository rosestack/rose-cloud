 ...
 *             ManagementFactory#getRuntimeMXBean() RuntimeMXBean}, which is a well-known, yet
 *             undocumented "hack", since most JVMs tend to use the JVM's PID as part of that MXBean
 *             name;
 *         <li>If all of the above fails, a random integer will be generated and used as a surrogate
 *             PID.
 *       </ol>
 * </ol>
 *
...
public final class UUIDs {
    ...
    private static String getProcessPiece() {
        ...
        //                if (pid == 0) {
        //                    LOGGER.warn(
        //                        "PID returned through native call was 0, JNR versions incompatible?  Falling back to
        // JMX.");
        //                    pid = null;
        //                } else {
        ...
        if (pid == null) {
            if (processHandleClass != null) {
                try {
                    ...
                    pid = Math.toIntExact((long) pidMethod.invoke(currentHandle));
                } catch (Exception e) {
                    LOGGER.warn("Failed to obtain PID from java 9", e);
                }
            }
        }
        if (pid == null) {
    ...
    /**
     * 检测当前 JDK 版本是否为 9 或更高
     */
    private static boolean isJava9OrHigher() {
        String version = System.getProperty("java.version");
        if (version.startsWith("1.")) { // Java 8 及以下版本格式为 1.x.x
            return false;
        }
        // 解析主版本号（例如 "9"、"11"、"17" 等）
     ...
     * <p>
     * copied from infinispan Util.java
     */
     ...
     * {@link System#currentTimeMillis} or {@link java.util.Date#getTime}), and <em>not</em> a count
     ...
     * System#currentTimeMillis}, {@link java.util.Date#getTime}, etc.
     *
    ...
    private static long getCurrentTimestamp() {
        while (true) {
            ...
            if (now > last) {
                ...
                if (millisOf(candidate) == lastMillis && lastTimestamp.compareAndSet(last, candidate)) return candidate;
            }
        }
    }
    ...
    private static Set<String> getAllLocalAddresses() {
        ...
        try {
            ...
            if (allMyIps != null) {
                for (int i = 0; i < allMyIps.length; i++) allIps.add(allMyIps[i].toString());
            }

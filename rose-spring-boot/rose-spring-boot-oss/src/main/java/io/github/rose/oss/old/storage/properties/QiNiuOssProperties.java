...
public class QiNiuOssProperties extends BaseOssProperties {
    ...
    public static class QiNiuStrategy {
        ...
        /**
         * 上传凭证有效截止时间。Unix时间戳，单位为秒。该截止时间为上传完成后，在七牛空间生成文件的校验时间，而非上传的开始时间，一般建议设置 为上传开始时间 +
         * 3600s，用户可根据具体的业务场景对凭证截止时间进行调整。
         **/
         ...
         * returnUrl?upload_ret=queryString，queryString 包含returnBody 内容。 如不设置
         * returnUrl，则直接将 returnBody 的内容返回给客户端。
         **/
        private String returnUrl;

        /**
         * 上传成功后，自定义七牛云最终返回給上传端（在指定 returnUrl 时是携带在跳转路径参数中）的数据。
         * 支持魔法变量和自定义变量。returnBody要求是合法的 JSON 文本。 例如 {"key": $(key),"hash": $(etag), "w":
         * $(imageInfo.width), "h": $(imageInfo.height)}。
         **/
        private String returnBody;

        /**
         * 上传成功后，七牛云向业务服务器发送 POST 请求的 URL。必须是公网上可以正常进行 POST 请求并能响应 HTTP/1.1 200 OK 的有效
         * URL。 另外，为了给客户端有一致的体验，我们要求callbackUrl 返回包 Content-Type 为 "application/jackson"，
         * 即返回的内容必须是合法的 JSON 文本。出于高可用的考虑，本字段允许设置多个 callbackUrl（用英文符号 ;分隔）， 在前一个
         * callbackUrl 请求失败的时候会依次重试下一个 callbackUrl。
         * 一个典型例子是：http://ip1/callback， 并同时指定下面的callbackHost 字段。 在
         * callbackUrl 中使用 ip 的好处是减少对 dns 解析的依赖， 可改善回调的性能和稳定性。 指定 callbackUrl，必须指定
         * callbackbody，且值不能为空。
         **/
         ...
         * 业务服务器可以通过直接读取请求的 query来获得该字段，支持魔法变量和自定义变量。 callbackBody 要求是合法的 url query
         * string。 例如key=$(key)&hash=$(etag)&w=$(imageInfo.width)&h=$(imageInfo.height)。
         * 如果callbackBodyType指定为application/jackson，则callbackBody应为json格式，
         * 例如:{"key":"$(key)","hash":"$(etag)","w":"$(imageInfo.width)","h":"$(imageInfo.height)"}。
         **/
        private String callbackBody;

        /**
         * 上传成功后，七牛云向业务服务器发送回调通知 callbackBody 的 Content-Type。 默认为
         * application/x-www-form-urlencoded，也可设置为 application/jackson。
         **/
        private String callbackBodyType;

        /**
         * 资源上传成功后触发执行的预转持久化处理指令列表。 每个指令是一个 API 规格字符串，多个指令用;分隔。请参阅persistenOps详解与示例。 同时添加
         * persistentPipeline字段，使用专用队列处理，请参阅persistentPipeline。
         **/
        private String persistentOps;

        /**
         * 接收持久化处理结果通知的 URL。必须是公网上可以正常进行 POST 请求并能响应 HTTP/1.1 200 OK 的有效 URL。 该 URL
         * 获取的内容和持久化处理状态查询的处理结果一致。 发送 body 格式是Content-Type 为 application/jackson 的 POST
         * 请求， 需要按照读取流的形式读取请求的 body 才能获取。
         **/
         ...
         * 值，如上传端指定了 MimeType 则直接使用该值，否则按如下顺序侦测 MimeType 值： 1. 检查文件扩展名； 2. 检查 Key 扩展名； 3.
         * 侦测内容。 如不能侦测出正确的值，会默认使用 application/octet-stream。
         **/
         ...
         * !application/jackson;text/plain表示禁止上传json文本和纯文本。 注意最前面的感叹号！
         **/

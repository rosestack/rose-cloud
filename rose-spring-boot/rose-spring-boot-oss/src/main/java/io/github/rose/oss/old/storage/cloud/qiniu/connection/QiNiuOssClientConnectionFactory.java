 ...
 */
package io.github.rose.oss.old.storage.cloud.qiniu.connection;
...
public class QiNiuOssClientConnectionFactory implements QiNiuConnectionFactory {
    ...
    public String getUploadToken(String bucket, String key, QiNiuScope scope) {
        Map<String, QiNiuOssProperties.QiNiuStrategy> strategies = qiNiuOssProperties.getStrategies();
        QiNiuOssProperties.QiNiuStrategy strategy = strategies.get(bucket);
        long expires = 3600L;
        boolean strict = qiNiuOssProperties.getStrict();
        if (strategy == null) {
            // 生成上传凭证
    ...
    /**
     * // 指定上传的目标资源空间 Bucket 和资源键 Key（最大为 750 字节）。有三种格式： // <bucket>，表示允许用户上传文件到指定的bucket。
    ...
    private StringMap createPolicy(QiNiuOssProperties.QiNiuStrategy strategy) {
